...
// Continuar con el contenido del índice del administrador
?>
<!doctype html>
<html lang="en">
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link" href="http://localhost/PROMEDIC/ROL_ADMIN/estadisticas/estadisticas.php?categoria_id=">Estadísticas</a>
          </li>
          <li class="nav-item dropdown">
            ...
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="http://localhost/PROMEDIC/ROL_ADMIN/Crud/promocion/promocion.php">Promociones</a></li>
              <li><a class="dropdown-item" href="http://localhost/PROMEDIC/ROL_ADMIN/Crud/AgregarProducto/Agregar.php">Agregar Producto</a></li>
            </ul>
          </li>
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
...
<script>
  ...
  // Función que extrae el texto del objeto notificación si es que es un objeto
  const obtenerTexto = (notification) => {
    // Si es objeto y tiene la propiedad "mensaje", devuélvela; de lo contrario, devuelve la notificación directamente.
  ...
  // Cargar notificaciones de ambos archivos
  const loadNotifications = () => {
    ...
    Promise.all([
      ...
      .then(([getNotifications, userNotifications]) => {
        ...
        // Mostrar notificaciones de get_notifications.php
        getNotifications.forEach(notification => {
          const mensaje = obtenerTexto(notification);
          const item = document.createElement('p');

          // Determinar el tipo de notificación y asignar un color en base al mensaje
          let iconColor;
          if (mensaje.includes('ha vencido')) {
            iconColor = 'red'; // Vencimiento
          } else if (mensaje.includes('por debajo de la cantidad mínima')) {
            iconColor = 'orange'; // Cantidad mínima
          } else if (mensaje.includes('superado la cantidad máxima')) {
            iconColor = 'green'; // Cantidad máxima
          } else {
            iconColor = 'yellow'; // Notificación genérica
          }

          // Crear el elemento de notificación
          item.innerHTML = `
            <span class="bi bi-bell-fill" style="font-size: 1.2rem; color: ${iconColor};"></span> 
            ${mensaje}`;
        ...
        // Mostrar notificaciones de notificaciones.php (solo las que no sean null)
        userNotifications.forEach(notification => {
          if (notification) {
            const mensaje = obtenerTexto(notification);
            const itemContainer = document.createElement('div');
            itemContainer.className = "d-flex align-items-center justify-content-between mb-2";
            ...
            deleteBtn.innerHTML = `<span class="bi bi-trash-fill" style="font-size: 1rem; color: red;"></span>`;
            deleteBtn.className = "btn btn-sm delete-btn";
            deleteBtn.onclick = () => {
              deleteNotification(mensaje, itemContainer);
            };

            itemContainer.appendChild(item);
            itemContainer.appendChild(deleteBtn);
            notificationBody.appendChild(itemContainer);
            totalNotifications++;
          }
        });
  ...
  document.getElementById('addNotification').addEventListener('click', () => {
    ...
    if (newNotification) {
      fetch('notificaciones.php', {
        ...
        body: `mensaje=${encodeURIComponent(newNotification)}`
      })
        .then(res => res.json())
        .then(data => {
        ...
        .catch(error => console.error('Error al agregar la notificación:', error));
    }
  });

  // Eliminar una notificación
  const deleteNotification = (mensaje, itemContainer) => {
    fetch('notificaciones.php', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    fetch("todos_los_productos.php") // Asegúrate de tener esta ruta para obtener todos los productos
        ...
        .catch(error => {
            ...
            <div class="dropdown">
              ...
              <li><a class="dropdown-item" href="http://localhost/PROMEDIC/Rol_Admin/regente_bd/interfaz_regente.php">Gestión de regente</a></li>
                <li><a class="dropdown-item" href="http://localhost/PROMEDIC/Rol_Admin/proveedor_bd/interfaz_proveedor.php">Gestión de Proveedor</a></li>
    ...
    <div class="container-fluid">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
            ...
            <div class="offcanvas-body">
                <!-- Barra de búsqueda -->
                <input class="form-control mb-3" id="search-bar" type="text" placeholder="Buscar categorías o clasificaciones..." oninput="filterCategoriesAndClassifications()">
...
<!-- Estilos inline -->
<style>
    #category-description, #classification-description {
        word-wrap: break-word;
        white-space: normal; /* Asegura que el texto largo se ajuste */
        overflow: visible;   /* Evita que el texto se oculte si es largo */
    }
</style>
...
<script>
    ...
    fetch('get_data.php')
        ...
        .then(data => {
            ...
            data.categorias.forEach(categoria => {
                ...
                li.innerHTML = `
                    <a class="nav-link" href="#" onclick="toggleDescription('category-description-${categoria.Id_Categoria}')">
                        ${categoria.Nombre_Categoria}
                    </a>
                    <div id="category-description-${categoria.Id_Categoria}" class="collapse mt-2">
                        <div class="card card-body position-relative">
                            <p class="mb-2">${categoria.Descripcion_Categoria}</p>
                            <button class="btn btn-link position-absolute top-0 end-0" onclick="toggleOptions('category-options-${categoria.Id_Categoria}')">
                                <i class="bi bi-three-dots"></i>
                            </button>
                            <div id="category-options-${categoria.Id_Categoria}" class="options" style="display: none;">
                                <div class="d-flex justify-content-end">
                                    <button class="btn btn-sm btn-outline-primary me-2" onclick="editCategory(${categoria.Id_Categoria}, '${categoria.Nombre_Categoria}', '${categoria.Descripcion_Categoria}')">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <button class="btn btn-sm btn-outline-danger" onclick="deleteCategory(${categoria.Id_Categoria})">
            ...
            data.clasificaciones.forEach(clasificacion => {
                ...
                li.className = 'nav-item';
                li.innerHTML = `
                    <a class="nav-link" href="#" onclick="toggleDescription('classification-description-${clasificacion.Id_Clasificacion}')">
                        ${clasificacion.Nombre_Clasificacion}
                    </a>
                    <div id="classification-description-${clasificacion.Id_Clasificacion}" class="collapse mt-2">
                        <div class="card card-body position-relative">
                            <p class="mb-2">${clasificacion.Descripcion_Clasificacion}</p>
                            <button class="btn btn-link position-absolute top-0 end-0" onclick="toggleOptions('classification-options-${clasificacion.Id_Clasificacion}')">
                                <i class="bi bi-three-dots"></i>
                            </button>
                            <div id="classification-options-${clasificacion.Id_Clasificacion}" class="options" style="display: none;">
                                <div class="d-flex justify-content-end">
                                    <button class="btn btn-sm btn-outline-primary me-2" onclick="editClassification(${clasificacion.Id_Clasificacion}, '${clasificacion.Nombre_Clasificacion}', '${clasificacion.Descripcion_Clasificacion}')">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <button class="btn btn-sm btn-outline-danger" onclick="deleteClassification(${clasificacion.Id_Clasificacion})">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                </div>
                            </div>
    ...
    // Función para mostrar u ocultar la descripción de la categoría o clasificación
    ...
    // Mostrar u ocultar las opciones de editar/eliminar
    ...
    // Función para editar categoría (nombre y descripción)
    function editCategory(id, currentName, currentDescription) {
        ...
        const newDescription = prompt("Ingrese la nueva descripción de la categoría:", currentDescription);
        ...
        if (newName && newDescription) {
            fetch('update_category.php', {
                ...
                body: JSON.stringify({
                    ...
                    descripcion_categoria: newDescription
                })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    ...
                    // Actualizar directamente los elementos en la interfaz sin recargar la página
    ...
    // Función para editar clasificación (nombre y descripción)
    function editClassification(id, currentName, currentDescription) {
        const newName = prompt("Ingrese el nuevo nombre de la clasificación:", currentName);
        const newDescription = prompt("Ingrese la nueva descripción de la clasificación:", currentDescription);

        if (newName && newDescription) {
            fetch('update_classification.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    id_clasificacion: id,
                    nombre_clasificacion: newName,
                    descripcion_clasificacion: newDescription
                })
            })
            .then(response => response.json())
            .then(data => {
    ...
    function deleteClassification(id) {
        if (confirm("¿Estás seguro de que deseas eliminar esta clasificación?")) {
            fetch('delete_classification.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ id_clasificacion: id })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    // Función para filtrar categorías y clasificaciones
    function filterCategoriesAndClassifications() {
        const query = document.getElementById('search-bar').value.toLowerCase();
        
        const categoryItems = document.querySelectorAll('#category-list .nav-item');
        const classificationItems = document.querySelectorAll('#classification-list .nav-item');
        
        categoryItems.forEach(item => {
            const name = item.querySelector('.nav-link').textContent.toLowerCase();
...
if ($result && $result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    ...
    // Si el producto está "No Disponible", lo agregamos a la lista de productos no disponibles
    if ($nombre_estado == 'No Disponible') {
      ...
      // Si el producto no está "No Disponible", lo agrupamos normalmente
...
// Imprimir los productos no disponibles al final de la tabla
