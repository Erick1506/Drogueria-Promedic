...
if (!isset($_SESSION['regente_id']) && !isset($_SESSION['admin_id'])) {
    $_SESSION['error_mensaje'] = "Ups, no has puesto el correo ni la contraseña.";
...
document.getElementById('goToDetailsBtn').addEventListener('click', () => {
    ...
    window.location.href = 'http://localhost:3001'; // Puedes cambiar esta URL si es necesario
});
</script>
...
<script>
    ...
    // Determinar el tipo de notificación y asignar un color
    let iconColor;
    if (notification.includes('ha vencido')) {
        iconColor = 'red'; // Vencimiento
    } else if (notification.includes('por debajo de la cantidad mínima')) {
        iconColor = 'orange'; // Cantidad mínima
    } else if (notification.includes('superado la cantidad máxima')) {
    ...
    item.innerHTML = `
        ...
        ${notification}`;
    item.className = "system-notification"; // Clase general para estilos adicionales
    ...
    totalNotifications++;
                ...
                userNotifications.forEach(notification => {
                    if (notification) {
                        ...
                        itemContainer.className = "d-flex align-items-center justify-content-between mb-2"; // Flexbox para alinear
                        
                        const item = document.createElement('p');
                        item.innerHTML = `<span class="bi bi-exclamation-triangle-fill" style="font-size: 1.2rem; color: orange;"></span> AVISO A REGENTE: ${notification}`;
                        item.className = "user-notification"; // Clase para diferenciar
                        item.style.flexGrow = '1'; // Permitir que el texto crezca
                        item.style.marginRight = '10px'; // Espacio entre el texto y el botón de eliminar
                        ...
                        deleteBtn.onclick = () => {
                            deleteNotification(notification, itemContainer);
                        };
            ...
            fetch('notificaciones.php', {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        document.getElementById('newNotification').value = ''; // Limpiar campo
                    } else {
                        alert(data.mensaje);
                    }
                })
    ...
    const deleteNotification = (mensaje, itemContainer) => {
        fetch('notificaciones.php', {
            ...
            .then(data => {
                if (data.status === 'success') {
                    itemContainer.remove();  // Eliminar el elemento del DOM
                    loadNotifications(); // Recargar notificaciones para actualizar el contador
                } else {
                    alert(data.mensaje);
                }
            })
    ...
    // Configurar evento para mostrar modal
  ...
  foreach ($productosAgrupados as $categoria => $clasificaciones) {
    ...
    foreach ($clasificaciones as $clasificacion => $productos) {
        ...
        foreach ($productos as $producto) {
            // Usar comillas dobles para permitir la interpolación de variables dentro del string
            echo "<tr>
